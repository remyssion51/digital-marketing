...
bullets = [
    ...
    f"• AOV (panier moyen) : {aov:,.0f} €".replace(","," "),
    f"• Dépenses marketing : {spend_total:,.0f} €".replace(","," "),
    f"• ROAS : {roas:.2f}" if roas is not None else "• ROAS : n/a"
]
st.write("\n".join(bullets))

st.subheader("Export Excel")
df_export = ord_f.copy()
output = BytesIO()
with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
    df_export.to_excel(writer, index=False, sheet_name="orders")
st.download_button("Télécharger Excel (commandes filtrées)", data=output.getvalue(), file_name="orders_export.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

st.caption("Pour un export PDF, convertissez une page HTML avec `pdfkit` ou utilisez ReportLab (à intégrer selon votre environnement).")